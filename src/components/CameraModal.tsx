        ...
        <div className="p-5 flex-1 overflow-y-auto space-y-4">
          {capturedPhoto ? (
            <div className="space-y-3">
              <div className="relative rounded-xl overflow-hidden border-2 border-[#0F9D58] shadow-md bg-black">
                <img src={capturedPhoto} alt="Captured check-in" className="w-full h-64 object-cover" />
                <div className="absolute bottom-2 left-2 right-2 bg-black/70 text-white text-xs px-3 py-1.5 rounded-md flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2.5 px-3 border border-gray-300 rounded-xl text-gray-700 font-medium text-sm flex items-center justify-center gap-1.5 hover:bg-gray-50 transition-colors"
                >
                  <RefreshCw className="w-4 h-4" /> Refaire
                </button>
                <button
                  onClick={confirmPhoto}
                  className="flex-1 py-2.5 px-3 bg-[#0F9D58] text-white font-medium text-sm rounded-xl flex items-center justify-center gap-1.5 hover:bg-[#0d8a4d] transition-colors shadow-sm"
                >
            ...
            <div className="space-y-4">
              {/* WebCam View or Fallback */}
              <div className="relative rounded-xl overflow-hidden bg-gray-900 border border-gray-200 aspect-4/3 flex items-center justify-center">
                {isCameraActive ? (
                  <>
                    <video
                      ref={videoRef}
                    ...
                    <div className="absolute top-3 left-3 bg-red-600 text-white text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1 animate-pulse">
                      <span className="w-2 h-2 rounded-full bg-white"></span> EN DIRECT
                    </div>
                  </>
                ) : (
                  <div className="p-6 text-center text-gray-400 space-y-2">
                    ...
                    <button
                      onClick={startCamera}
                      className="mt-2 text-xs text-[#0F9D58] font-semibold underline hover:text-[#0b7742]"
                    >
                      Réessayer la caméra
                    </button>
                  </div>
              ...
              {/* Take Photo Button if active */}
              {isCameraActive && (
                <button
                  onClick={takeSnapshot}
                  className="w-full py-3 bg-[#0F9D58] text-white font-semibold rounded-xl flex items-center justify-center gap-2 hover:bg-[#0b8046] transition-all shadow-md active:scale-98"
                >
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  className="w-full py-2.5 border border-dashed border-[#0F9D58] text-[#0F9D58] font-medium text-xs rounded-xl flex items-center justify-center gap-2 hover:bg-emerald-50 transition-colors"
                >
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  {PRESET_PHOTOS.map((preset, idx) => (
                    <button
                      ...
                      className="group relative rounded-lg overflow-hidden border border-gray-200 hover:border-[#0F9D58] transition-all text-left"
                    >
